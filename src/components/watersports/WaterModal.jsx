...
const WaterModal = ({ item, closeModal }) => {
      ...
      >
        <div className="min-w-[32vw]">
          ...
          <div className="p-4 sm:max-w-[500px] max-w-full mx-auto">
            <img src={item?.img} width={5488} height={3659} className="" />
            <h1 className="font-bold mt-5 text-xl">{item?.name}</h1>
            <p className="text-center mt-4 text-gray-700 text-[16px]">
              {item?.description}
            </p>
          </div>
          <div className="flex w-full items-center justify-evenly border-t-[1px] border-gray-300 pt-4">
            <Link
              to={'/Contact'}
              target="_blank"
              rel="noreferrer"
